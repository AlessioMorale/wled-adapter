...
class Tester():
    ...
    def run(self):
        ...
        config = State(seg=[
            Seg(id=0, start=0, stop=6, grp=1, on=True, bri=32, fx=0, 
        ...
        for pixel in range(4):
            for col in range(iterations):
                ...
                self._adapter.segments[segment]._set_pixel(pixel, Color("blue", luminance = 1 - colour))
                self._adapter.segments[segment]._set_pixel(pixel + 1, Color("purple", luminance = colour))
                self._adapter.segments[segment]._set_pixel(pixel + 2, Color("green", luminance = 1 - colour))
